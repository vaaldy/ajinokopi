...
phase 2 = press-drag-release drill, orbiting note pills, live fingerprint hub, SCA cupping sheet, brew-named export files (done)
...
- Phase 3 prerequisites, all manual. Done: `src/wheel.jsx` + `pnpm-lock.yaml` committed. Left: `git rm package-lock.json`, add `version` + `packageManager` to `package.json`, set repo Pages source to "GitHub Actions".
