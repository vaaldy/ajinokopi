...
Coffee taste-note wheel. React + Vite, static, no backend. Read this first. All other Markdown
lives in `prompts/`; read only what table below says you need — repo small, but blind grep still wastes tokens.
...
| **`/note-take`** | doc freshness | Run before + after every release. Owns `prompts/architecture.md` + `prompts/status.md`. Never runs git. |
...
| Need | File | Notes |
| --- | --- | --- |
| What exists now, why shaped that way | `prompts/architecture.md` | File map, **Vocabulary** (UI names), Blocks, data flow, conventions. Start here. |
| What is done / open | `prompts/status.md` | One line per version + Open list. Lean by design. |
...
| Vocabulary (hub, pill, drill, tone, fan…) | `prompts/architecture.md` → Vocabulary | Every UI name defined once. Check before inventing a term. |
| All pure logic + storage | `src/lib.js` | Geometry, colors, wheel layout, pill orbit, fingerprint math, brew store. No React. |
| SVG + pointer handling | `src/wheel.jsx` | `<Wheel>`, `<Fingerprint>`. Perf-sensitive — read `prompts/architecture.md` Blocks before touching. |
...
| Doc discipline, run before + after each release | `prompts/note-take.md` | Canonical `/note-take` skill. Owns `prompts/architecture.md` + `prompts/status.md`. Copy to `.claude/skills/note-take/SKILL.md` to make the command work — `.claude/` is gitignored. |
| Tests | `test.js` | `pnpm test`. Pure functions in `src/lib.js` only. `.jsx` has no coverage. |
| How to release / deploy, in human terms | `prompts/README.md` | Contributor-facing. Workflow table, walkthrough, gotchas. |
| Release pipeline | `.github/workflows/` | `ci.yml` (main + PRs), `tag.yml` (version bump cuts tag), `release.yml` (build, Pages, GitHub Release). Design notes in `prompts/architecture.md` Blocks. |
...
/note-take                 # after: prompts/architecture.md = reality, prompts/status.md
```
...
- Perf lives in the render tree, not the maths. CSS `filter` on an ancestor welds its whole subtree
  into one raster unit — see `prompts/architecture.md` Blocks.
