...
v0.3 = release CI/CD: version-bump auto-tagging, Pages deploy, GitHub Releases (planned — plan approved, nothing built)
...
- v0.3 prerequisites, all manual. Done: `src/wheel.jsx` + `pnpm-lock.yaml` committed, `version` set to 0.2.0. Left: tag `v0.2.0`, `git rm package-lock.json`, add `packageManager` to `package.json`, set repo Pages source to "GitHub Actions".
